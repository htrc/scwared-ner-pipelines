...
#%%timeit -n 1 -r 1
nlp = spacy.load('/N/u/ninoparu/Carbonate/slate/projects/spanish-ner-custom/dataset/es_output_2/model-best')
...
get_ipython().system('mkdir scwared-spanish-custom-2')
...
for l_dir in os.listdir("scwared-spanish-data"):
    get_ipython().system('mkdir scwared-spanish-custom-2/{l_dir}')
    for f_name in tqdm.tqdm(os.listdir(f"scwared-spanish-data/{l_dir}")):
        # skip if entity already exists
        result = []
        sentences = []
        vol_id = ".".join(f_name.split(".")[:-1])
        if os.path.exists(f"scwared-spanish-custom-2/{l_dir}/{vol_id}.csv"):
            continue
        ...
        result_pd.to_csv(f"scwared-spanish-custom-2/{l_dir}/{vol_id}.csv",index=False)
        ...
        sentences_pd.to_csv(f"scwared-spanish-custom-2/{l_dir}/sent-{vol_id}.csv",index=False)        
#end_time = time.time()
